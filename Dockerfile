...
# The demo corpus is NOT baked in. It is published to a public R2 bucket as a
# static coefficient HTTP store (scripts/publish_corpus_r2.py) and selected at
# runtime with SQUINTLY_COEFFICIENT_HTTP — see DEPLOY.md §15. Baking it cost a
# 119 MB image layer and a 121 MB build-context upload on every deploy, and
# meant re-deploying the whole service to change one image.
