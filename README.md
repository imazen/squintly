...
> **Where the current state lives (2026-08-06).** This README describes intent;
> `CLAUDE.md` carries the live invariants and is kept current. For what shipped
> and when see `CHANGELOG.md`; for the observer-facing methodology (feedback,
> debriefs, and the first complete disposition reading) see
> `docs/OBSERVER-FEEDBACK.md`. `docs/HANDOFF.md` is stale as of 2026-05-01.
...
| Live | **https://squintly.imazen.org** — all four flows work: rating (180 imazen26-v4-test sources (imazen-26 TEST split only) across 21 strata / 4320 encodings across libjpeg-turbo, jpegli, libwebp, libavif, served straight from public R2 — see [`DEPLOY.md`](DEPLOY.md) §15), curator (903 R2 candidates), calibration, suggest. Confirm which build you are looking at with `curl .../api/export/pareto.manifest.json`. |
...
| **2** | **Viewing-conditions captured per trial** (DPR, intrinsic-to-device, calibrated CSS-px/mm, ambient, distance, gamut) | ✅ | `migrations/0001_init.sql`, `web/src/conditions.ts`, `web/src/calibration.ts` | Display is a hard **1:1 device pixels** minimum — never scaled down; anything larger is panned (`docs/methodology.md` §2). `intrinsic_to_device_ratio` is 1.0 by construction, and `pan_*`/`visible_*` (migration 0012) record how much was seen. |
...
| **4** | **Trivial-triplet filter** — pairs more than 15 ssim2 points apart are filtered out. *[note 2026-08-06: this row previously described a "metric ensemble unanimously predicts (≥95%)" design that was never built; the shipped filter was an encoder-metadata heuristic. It is now genuinely metric-driven — `sampling::is_trivial_pair_scored` against ingested `encoding_metrics`, with the heuristic as fallback for unscored encodings.]* | ✅ | [`docs/methodology.md`](docs/methodology.md) §3.4, `src/sampling.rs` | verify firing rate in pilot |
...
| **7** | **Honeypots** — known-answer trials at **1/12** (`p_honeypot = 0.083`) *[note 2026-08-06: this said ~1/30, which never matched the code]*; fail-two-end-session | ✅ | [`docs/methodology.md`](docs/methodology.md) §3.9 | tune fail rate to ≤ 5 % on T1 observers |
...
| **14** | **Boosted Triplet Comparison (BTC)** — quadratic boosting `h(d) = γ₁d + γ₂d²` for high-fidelity near-JND sensitivity | 🔵 | not yet | v0.3 optional add for the q ≥ 80 band; the paper reports scales larger by a factor of **about 2** with precision about twice as good — i.e. ~4× fewer trials at equal CI, not 3×/9× *[note 2026-08-06: corrected from an earlier "~3× discrimination" claim]* ([AIC-3 2410.09501](https://arxiv.org/abs/2410.09501)) |
