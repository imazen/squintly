...
| **12** | **Unified pairwise + ACR-rating scale** (Pérez-Ortiz 2019 mixed BT-Davidson + ordinal model) | 🟡 | `src/bt.rs`, `src/unified.rs` (solver currently diverges — see CLAUDE.md "Known Bugs"), [`docs/methodology.md`](docs/methodology.md) §5.5 | `pair_log_likelihood`/`rating_log_likelihood`/`total_log_likelihood` helpers shipped so the H3 pilot evaluation has a stable entry point even with the in-tree solver broken. The solver itself needs a rewrite (`grad_log_sigma_o` is NaN-producing, `σ` diverges, `τ` wanders) — until then, H3 evaluation calls our LL helpers against fits from the reference implementations under `/mnt/v/repos/iqa-tools/`. |
